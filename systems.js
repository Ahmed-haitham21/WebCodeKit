...
function openSystem(id) {
  ...
  // نحمي الصفحة الرئيسية من أي CSS عام داخل نظام معين
  // بنغلف المحتوى داخل div ثابت ونطبّق CSS على نطاقه فقط
  const scopedCss = String(system.css)
    // أهم سطر: أي `body { ... }` لا ينعكس على صفحة systems.html
    .replace(/\bbody\s*\{/g, "#systemInner body {")
    // scoping لـ html
    .replace(/\bhtml\s*\{/g, "#systemInner html {")
    // scoping لـ reset العام (يؤثر داخل النظام فقط)
    .replace(/\*\s*\{/g, "#systemInner * {")
    // لو في CSS فيه selectors عامة بتستهدف عناصر مباشرة (مش body فقط) نخليها على نطاق النظام
    .replace(
      /(^|\})\s*(h1|h2|h3|p|input|textarea|button|div|ul|li|table|th|td)\s*\{/g,
      (m, br, tag) => {
        return `${br} #systemInner ${tag} {`;
      },
    );
  ...
  // تشغيل JavaScript الخاصة بالنظام بعد إدراج الـ DOM
  // مهم: عناصر النظام تكون موجودة قبل تشغيل الـ JS
  try {
    const systemInner = viewContainer.querySelector("#systemInner");
    const js = String(system.js);

    // 1) نوفّر document/querySelector داخل #systemInner
    // 2) نعمل global exposure للدوال المطلوبة ل onclick="..." داخل الـ HTML
    // علشان كود onclick ينفّذ بدون ما يفشل بسبب scope.
    const runner = new Function(
      "systemInner",
      `
      const root = systemInner;

      // document محصور داخل systemInner
      const document = {
      ...
      // حقن الدوال التي تخرج من JS عشان onclick يشوفها على مستوى window
      // (زي appendToDisplay / calculate / currentSlide / etc)
      const expose = (obj) => {
        if (!obj || typeof obj !== 'object') return;
        Object.keys(obj).forEach((k) => {
          if (typeof obj[k] === 'function') window[k] = obj[k];
        });
      };

      // نفّذ سكربت النظام
      // لو السكربت عرّف وظائف بـ function name() أو const name = ...
      // هنحاول نجمعها بعد التنفيذ.
      ${js}

      // Expose: نحاول التقاط دوال شائعة متوقعة من onclick
      // (مش كل الأنظمة تستخدمها، لكن ده بيغطي Calculator/Slider)
      expose({
        appendToDisplay,
        clearDisplay,
        calculate,
    ...
    // بعد التنفيذ، فعّل onclick handlers الخاصة بالـ HTML إن وُجدت عبر new Function
    // ملاحظة: onclick داخل HTML هيكون على مستوى window؛ فلو الدوال اتعرضت هتشتغل.
  } catch (e) {
    console.error("System JS failed:", e);
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  filterBtns.forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      const items = document.querySelectorAll(".system-card");
      let visibleCount = 0;

...
buttonData[33] = {
  html: `<a class="btn-33" href="https://mail.google.com/mail/?view=cm&fs=1&to=ahmed.haitham2005@icloud.com" target="_blank" rel="noopener noreferrer"><span class="btn-icon"><i class="fas fa-at"></i><span>Email Us</span></span></a>`,
  css: `.btn-33 { padding: 14px 30px; border: none; border-radius: 25px; background: #ffff00; color: #333; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 10px; transition: all 0.3s; text-decoration: none; }`,
};
...
<body>
  ${btnInfo.html}
      ...
      // Add Prism language classes
      document.getElementById("htmlCode").className = "language-html";
      document.getElementById("cssCode").className  = "language-css";
      document.getElementById("jsCode").className   = "language-javascript";
...
function copyAllCode() {
  ...
  let fullCode = "";
  if (htmlCode.trim()) fullCode += `<!-- HTML -->\n${htmlCode}\n\n`;
  if (cssCode.trim())  fullCode += `/* CSS */\n${cssCode}\n\n`;
  if (jsCode.trim())   fullCode += `/* JavaScript */\n${jsCode}`;

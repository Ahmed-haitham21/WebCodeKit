...
const AI_CONFIG = {
  useAPI:     true,
  backendUrl: "",
  apiKey:     localStorage.getItem("wck_api_key") || "",
};
...
async function sendMessage() {
  ...
  const finalText = text || 'حلل هذا التصميم واعمل كوده';
  ta.value = '';
  ta.style.height = 'auto';

  // If image uploaded — use Vision mode
  if (uploadedImageBase64) {
    appendUserMsg(`📸 ${finalText}`);
    addToHistory('user', finalText);
    const typing = appendBotTyping();

    const visionResult = await callVisionAI(finalText, uploadedImageBase64, uploadedImageType);
    clearImage();
    typing.remove();

    if (visionResult?.__error__?.includes('401') || visionResult?.__error__?.includes('authentication') || (!AI_CONFIG.apiKey && visionResult?.__error__)) {
      appendBotMsg(`🔑 <strong>محتاج API Key</strong> — اضغط <button onclick="openApiSettings()" style="background:rgba(102,126,234,0.2);border:1px solid rgba(102,126,234,0.4);color:#a5b4fc;border-radius:6px;padding:2px 10px;font-size:12px;cursor:pointer">🔑 Add API Key</button> عشان تستخدم Design-to-Code.`);
    } else if (visionResult?.__error__) {
      appendBotMsg(`⚠️ <strong>حصل خطأ:</strong> ${visionResult.__error__}<br><small>جرب صورة أصغر أو تأكد من صحة الـ API key.</small>`);
    } else if (visionResult?.html) {
      lastCodeResult = visionResult;
      appendBotMsg(`✨ <strong>Design-to-Code جاهز!</strong><br>حللت الصورة وولدت الكود المطابق. اضغط <strong>Playground</strong> لتعديله.`);
      renderCodeOutput(visionResult);
    } else {
      appendBotMsg('⚠️ مش قدرت أحلل الصورة. جرب صورة UI أوضح.');
    }
    return;
  }
  ...
  if (isCodeRequest(finalText)) {
    // CODE MODE
    if (useAPI) await delay(400);
    const apiResult = useAPI ? await callCodeAI(finalText) : null;

    typing.remove();

    if (apiResult === '__SERVER_DOWN__') {
      appendBotMsg('⚡ <strong>Demo Mode</strong> — الـ AI server مش متاح، بس عندي قوالب جاهزة 👇');
      const fallback = matchTemplate(text);
      lastCodeResult = fallback;
      renderCodeOutput(fallback);
    } else {
      const result = apiResult || matchTemplate(text);
    ...
    if (!reply || reply === '__SERVER_DOWN__') {
      if (reply === '__SERVER_DOWN__') {
        reply = '⚡ <strong>Demo Mode</strong> — الـ AI server مش متاح، بس هجاوب من المعلومات الجاهزة 👇<br><br>' + getFallbackReply(text);
      } else {
...
function handleImageUpload(input) {
  ...
  reader.onload = (e) => {
    ...
    // Show preview bar
    document.getElementById('imageThumb').src = dataUrl;
    document.getElementById('imagePreviewBar').style.display = 'flex';

    // Auto-fill prompt if empty
    const ta = document.getElementById('aiPrompt');
    if (!ta.value.trim()) {
      ta.value = 'حلل هذا التصميم واعمل كوده بـ HTML+CSS';
      ta.style.height = 'auto';
...
async function callVisionAI(prompt, imageBase64, imageType) {
  // Try server first
  if (AI_CONFIG.backendUrl !== '__direct__') {
    try {
      const res = await fetch(`${AI_CONFIG.backendUrl}/api/vision`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt, imageBase64, imageType })
      });
      if (res.ok) return await res.json();
    } catch (e) {
      console.warn('Server not available, trying direct API...');
    }
  }
  ...
  try {
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 4000,
        system: `You are a Design-to-Code expert. Analyze the UI screenshot and generate matching code.
...
- Make it responsive
- Return complete standalone code`,
        messages: [
          {
            role: 'user',
            content: [
              ...
              {
                type: 'text',
                text: prompt || 'Analyze this UI design and generate matching HTML+CSS+JS code.',
              },
            ],
          },
        ],
    ...
    console.error('Direct Vision API error:', e);
...
function enablePlayground() {
  // Convert code display to editable textareas
  const sections = [
    { id: 'htmlCode', lang: 'HTML', color: '#e34c26' },
    { id: 'cssCode',  lang: 'CSS',  color: '#264de4' },
    { id: 'jsCode',   lang: 'JS',   color: '#f7df1e' },
  ];

  sections.forEach(({ id, lang, color }) => {
    const el = document.getElementById(id);
    if (!el) return;
    const currentCode = el.textContent;

    const wrapper = document.createElement('div');
    const label = document.createElement('div');
    label.style.cssText = `font-size:11px;font-weight:700;color:${color};margin-bottom:6px;text-transform:uppercase;letter-spacing:0.5px`;
    label.textContent = lang;

    const textarea = document.createElement('textarea');
    textarea.className = 'playground-textarea';
    textarea.value = currentCode;
    textarea.id = id + '_edit';
    textarea.setAttribute('spellcheck', 'false');
    textarea.setAttribute('autocomplete', 'off');

    // Live update iframe on input
  ...
  const existingToolbar = document.getElementById('playground-toolbar');
  if (!existingToolbar) {
    const toolbar = document.createElement('div');
    toolbar.id = 'playground-toolbar';
    toolbar.className = 'playground-toolbar';
    toolbar.innerHTML = `
      <button class="playground-btn playground-btn-run" onclick="updatePlaygroundPreview()">
        <i class="fas fa-play"></i> Run
      </button>
      <button class="playground-btn playground-btn-reset" onclick="resetPlayground()">
        <i class="fas fa-undo"></i> Reset
      </button>
      <button class="playground-btn playground-btn-copy" onclick="copyPlaygroundCode()">
        <i class="fas fa-copy"></i> Copy All
      </button>
      <span class="playground-hint">✏️ Edit code and see live changes</span>
    `;
...
let _originalCode = null;

function resetPlayground() {
  if (!lastCodeResult) return;
  ['html','css','js'].forEach(lang => {
    const el = document.getElementById(lang + 'Code_edit');
    if (el) el.value = lastCodeResult[lang] || '';
  });
  updatePlaygroundPreview();
}

function updatePlaygroundPreview() {
  const html = document.getElementById('htmlCode_edit')?.value || document.getElementById('htmlCode')?.textContent || '';
  const css  = document.getElementById('cssCode_edit')?.value  || document.getElementById('cssCode')?.textContent  || '';
  const js   = document.getElementById('jsCode_edit')?.value   || document.getElementById('jsCode')?.textContent   || '';
  ...
  iframe.srcdoc = `<!DOCTYPE html><html><head><meta charset="UTF-8">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
<style>body{margin:16px;padding:0;background:#0f1219;display:flex;align-items:center;justify-content:center;min-height:calc(100vh - 32px)}${css}</style>
</head><body>${html}<script>${js.replace(/<\/script>/gi,'<\\/script>')}<\/script></body></html>`;
}

function copyPlaygroundCode() {
  const html = document.getElementById('htmlCode_edit')?.value || '';
  const css  = document.getElementById('cssCode_edit')?.value  || '';
  const js   = document.getElementById('jsCode_edit')?.value   || '';
  const out  = `<!-- HTML -->\n${html}\n\n/* CSS */\n${css}${js ? `\n\n// JS\n${js}` : ''}`;
  navigator.clipboard.writeText(out).then(() => showToast('All code copied! 🎉'));
}
...
function saveApiKey() {
  ...
  if (!key) {
    if (status) { status.style.display = 'block'; status.style.color = '#ef4444'; status.textContent = '⚠️ من فضلك أدخل API key'; }
    return;
  }
  if (!key.startsWith('sk-ant-')) {
    if (status) { status.style.display = 'block'; status.style.color = '#f59e0b'; status.textContent = '⚠️ المفتاح لازم يبدأ بـ sk-ant-'; }
    return;
  }

  AI_CONFIG.apiKey = key;
  localStorage.setItem('wck_api_key', key);

  if (status) {
    status.style.display = 'block';
    status.style.color = '#22c55e';
    status.textContent = '✅ تم حفظ المفتاح! دلوقتي تقدر تستخدم Design-to-Code';
  }

  // Hide server notice, update UI
  document.getElementById('server-notice').style.display = 'none';
  setTimeout(() => closeApiSettings(), 1500);
}

function clearApiKey() {
  AI_CONFIG.apiKey = '';
  localStorage.removeItem('wck_api_key');
  const input = document.getElementById('apiKeyInput');
  if (input) input.value = '';
  const status = document.getElementById('apiKeyStatus');
  if (status) { status.style.display = 'block'; status.style.color = '#ef4444'; status.textContent = '🗑️ تم مسح المفتاح'; }
}

// Close modal on outside click
document.addEventListener('click', (e) => {
  const modal = document.getElementById('apiSettingsModal');
  if (modal && e.target === modal) closeApiSettings();
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Check if server is available — show demo notice if not
  if (AI_CONFIG.useAPI) {
    fetch(`${AI_CONFIG.backendUrl}/api/chat`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
